# Contributions

Registre fork-only conservé sur `custom`. Git porte le détail technique; ce fichier porte l'état de livraison et l'accord humain. Une ligne ne passe à `Approved` qu'après un test explicite de Nicolas sur l'image indiquée.

États : `Development` → `Validated` → `On custom` → `Deployed` → `Tested` → `Approved` → `PR opened` → `Merged`. `Changes requested` revient à `Development`; `Blocked` doit nommer la cause.

| ID | Feature | Branche | Commit | Image Unraid | Validation Nicolas | PR | État / cause |
|---|---|---|---|---|---|---|---|
| DETAIL-003 | Plex, Radarr and Sonarr availability on details | `feat/detail-availability` | `b8d7f206` | `sha-f6346f9920926d40195104b25e6ebeab28f8f55c` | Pending | — | Deployed — index Plex complet lancé; conteneur sain, migrations appliquées et préflight Unraid vert |
| HISTORY-002 | Expandable cached episode groups | `fix/activity-journal-binge-cache` | `e6b879ac` | `sha-6457dc628bfa5f86f4235924b424f41b577bfa23` | Pending | — | Deployed — le groupe réel du 24 août affiche 3 épisodes, 151 minutes, la plage S03E01-E03 et son chevron; Home en 200 et préflight vert |
| PERF-001 | Bounded Home pagination and local search | `feat/general-performance` | `b755b07b` | `sha-76bdd889c682f402158bdf4502b8b484e9911a66` | Pending | — | Deployed — Home hydrate seulement la page demandée et les recherches locales sont limitées en SQL; conteneur sain et préflight vert |
| HOME-009 | Companion Home parity (Shelves, Activity Journal & Binge grouping) | `feat/home-all-media-in-progress` | `947772ca` | `sha-0e2755c030bd60dfaab3e4ee8ce0cbdd3ae2fafa` | Pending | — | Deployed — contrôles History lisibles, dates allégées et filtre placé en haut; 63 tests, conteneur sain et préflight Unraid verts |
| DETAIL-001 | Weighted public rating breakdown | `feat/detail-rating-aggregate` | `c479bfb2` | `sha-39037960b0c72543ac9a62d93531f9e206aaccba` | Pending | — | Deployed — calcul, affichage, options et préflight vert |
| DETAIL-002 | Public reviews from TMDB, BetaSeries and Hardcover | `fix/public-reviews-appearance` | `8249f726` | `sha-76bdd889c682f402158bdf4502b8b484e9911a66` | Pending | — | Deployed — les balises spoiler BetaSeries sont masquées et révélables volontairement; conteneur sain et préflight vert |
| PWA-001 | Installable app experience | `feat/pwa-install` | `0a810b38` | `sha-2f3741c8d86dc66a0edfe8b08597d5ecb418eddf` | Changes requested | — | Development — installation non fonctionnelle rapportée par Nicolas |
| UI-002 | Direct horizontal carousel gestures | `feat/horizontal-drag-upstream` | `30f10c04` | `sha-6a89a7fe49f462f8d99875cb4672248c802d5e2f` | Approved | [#1134](https://github.com/dannyvfilms/Floppy/pull/1134) | PR opened after Nicolas validated the Unraid deployment; double review, targeted tests, Ruff, browser drag/click QA and preflight passed |
| UI-003 | Home dropdown stacking | `fix/home-dropdown-stacking` | `01376542` | `sha-3e50935263dea21f97635e7cf269ce1d4d71513b` | Pending | — | Deployed — menus au-dessus des rangées voisines; 50 tests Home Screen, 9 tests de migrations, Ruff et préflight verts |
| UI-001A | Episode picker overflow | `fix/episode-picker-overflow` | `b5f171f9` | `sha-b1be5e1f235dc78a6fb9739a1373d0bc75b2e230` | Approved | [#1144](https://github.com/dannyvfilms/Floppy/pull/1144) | PR opened from `upstream/latest`; focused regression test, Ruff, diff check and mobile screenshot verified |
| UI-001B | Appearance data foundation | `feat/appearance-foundation` | `e2f5b110` | `sha-b1be5e1f235dc78a6fb9739a1373d0bc75b2e230` | Approved | [#1145](https://github.com/dannyvfilms/Floppy/pull/1145) | PR opened from `upstream/latest`; migration test, model check, Ruff and strict migration hygiene passed |
| UI-001C | Themes and detail layouts | `feat/appearance-themes` | `beb61ba3` | `sha-81216db295b9c651476927ecea3461560e756c51` | Approved | [#1155](https://github.com/dannyvfilms/Floppy/pull/1155) | Opened from `upstream/latest`; six focused screenshots, 26 targeted tests, Ruff, stable Tailwind output and preflight verified |
| UI-001D | Custom branding | `feat/custom-branding-clean` | `42b59747` | `sha-754e5b13aadca99727a485f2d548ad805f245501` | Pending | — | Deployed on Unraid from `custom` with healthy container, migration and Redis preflight, and verified backup; nine screenshots, 67 targeted custom tests, Ruff and strict migration hygiene pass; Nicolas's visual test and full Linux suite remain before PR approval |
| HOME-008 | Configurable Home history row | `feat/home-history-row` | `a024c225` | `sha-4eb358679e1a017cc754f1478defa145b16450f9` | Pending | — | Deployed — le mode All media conserve désormais tous les visionnages; 8 tests History Row, Ruff et préflight vert |
| HISTORY-001 | Consecutive episode grouping | `feat/history-consecutive` | `ab9821f7` | `sha-a6762b17070dd7e56c0cfa23fcea3ad455558334` | Pending | — | Deployed — regroupement par série et saison, 28 tests History et Ruff validés |
| HOME-006 | Cross-media `In progress` row | `feat/home-all-media-ready` | `971877ff` | `sha-39037960b0c72543ac9a62d93531f9e206aaccba` | Pending | — | Deployed — 82 tests ciblés, migration de fusion, Ruff et préflight vert |
| SERIES-001 | Series, seasons, credits and person details | `feat/home-all-media-in-progress` | `e8782740` | `sha-261dd9881146abb57b0412d7c9618bb7e753c383` | Pending | — | Deployed — cartes compactes et top 3 complété par les œuvres connues; conteneur sain, migrations appliquées et préflight Unraid vert |
| HOME-005 | Collapsible Home rows | `feat/home-collapsible-rows` | `c1aebfb9` | `sha-a9e52961` | Works | — | Tested |
| HOME-004 | `Up Next` row | `feat/home-up-next` | `0dd683ca` | `sha-a6762b17070dd7e56c0cfa23fcea3ad455558334` | Pending | — | Deployed — `SxxExx` visible et désactivable; `Now Playing` ouvre l’épisode avec option; 138 tests combinés et Ruff validés |
| HOME-003 | Stale Home row | `feat/home-stale-row` | `79267e47` | `sha-a9e52961` | Pending | — | Deployed |
| HOME-002 | Resume next-episode navigation | `feat/home-resume-navigation` | `62bd5b4a` | `sha-62bd5b4af655e678cc30dba6427c1367ff875660` | Pending | — | Deployed — l’ouverture directe de l’épisode est configurable sur la rangée All media In Progress et active par défaut; pages en 200 et préflight vert |
| HOME-001 | Pinned Home titles | `feat/home-pins` | `6e5a1d7e` | `sha-a9e52961` | Pending | — | Deployed |

## Règles de mise à jour

- Le coordinateur met à jour la ligne au début et à chaque changement d'état.
- Le commit est le commit propre de la branche; l'image est le tag immuable réellement déployé.
- `Tested` décrit le verdict de Nicolas, jamais seulement les tests automatisés.
- `Approved` autorise uniquement la PR indiquée; aucun accord global ou implicite.
- Après merge upstream, enregistrer le lien de PR puis retirer les détails devenus inutiles du `TODO.md`.
