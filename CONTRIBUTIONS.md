...
|---|---|---|---|---|---|---|---|
| DETAIL-003 | Plex, Radarr and Sonarr availability on details | `feat/detail-availability` | `b8d7f206` | `sha-f6346f9920926d40195104b25e6ebeab28f8f55c` | Pending | — | Deployed — index Plex complet lancé; conteneur sain, migrations appliquées et préflight Unraid vert |
...
| HOME-009 | Companion Home parity (Shelves, Activity Journal & Binge grouping) | `feat/home-all-media-in-progress` | `947772ca` | `sha-0e2755c030bd60dfaab3e4ee8ce0cbdd3ae2fafa` | Pending | — | Deployed — contrôles History lisibles, dates allégées et filtre placé en haut; 63 tests, conteneur sain et préflight Unraid verts |
...
| DETAIL-002 | Public reviews from TMDB, BetaSeries and Hardcover | `fix/public-reviews-appearance` | `8249f726` | `sha-76bdd889c682f402158bdf4502b8b484e9911a66` | Pending | — | Deployed — les balises spoiler BetaSeries sont masquées et révélables volontairement; conteneur sain et préflight vert |
...
| UI-002 | Direct horizontal carousel gestures | `feat/horizontal-drag-upstream` | `30f10c04` | `sha-b72a7294f537a6de22232d840d6573f8f9084bdf` | Approved | [#1134](https://github.com/dannyvfilms/Floppy/pull/1134) | PR opened after Nicolas validated the Unraid deployment; upstream fix #1139 ported to `custom`, 5 targeted tests, Ruff, healthy container and remote preflight passed |
...
| UI-001A | Episode picker overflow | `fix/episode-picker-overflow` | `b5f171f9` | `sha-b1be5e1f235dc78a6fb9739a1373d0bc75b2e230` | Approved | [#1144](https://github.com/dannyvfilms/Floppy/pull/1144) | PR opened from `upstream/latest`; focused regression test, Ruff, diff check and mobile screenshot verified |
| UI-001B | Appearance data foundation | `feat/appearance-foundation` | `e2f5b110` | `sha-b1be5e1f235dc78a6fb9739a1373d0bc75b2e230` | Approved | [#1145](https://github.com/dannyvfilms/Floppy/pull/1145) | PR opened from `upstream/latest`; migration test, model check, Ruff and strict migration hygiene passed |
| UI-001C | Themes and detail layouts | `feat/appearance-themes` | `beb61ba3` | `sha-81216db295b9c651476927ecea3461560e756c51` | Approved | [#1155](https://github.com/dannyvfilms/Floppy/pull/1155) | Opened from `upstream/latest`; six focused screenshots, 26 targeted tests, Ruff, stable Tailwind output and preflight verified |
| UI-001D | Custom branding | `feat/custom-branding-final` | `cb7f8255` | `sha-d4d402e0699f5337fa3702a5eacdf5fff3208bb1` | Open | [#1234](https://github.com/dannyvfilms/Floppy/pull/1234) | Deployed and validated on Unraid; Save appearance updates the sidebar and signed-out logo and theme; 61 focused PR tests, 72 integrated tests, Ruff, migration checks and nine browser captures pass; Linux CI is the remaining gate |
...
| SERIES-001 | Series, seasons, credits and person details | `feat/home-all-media-in-progress` | `e8782740` | `sha-261dd9881146abb57b0412d7c9618bb7e753c383` | Pending | — | Deployed — cartes compactes et top 3 complété par les œuvres connues; conteneur sain, migrations appliquées et préflight Unraid vert |
...
| HOME-002 | Resume next-episode navigation | `feat/home-resume-navigation` | `62bd5b4a` | `sha-62bd5b4af655e678cc30dba6427c1367ff875660` | Pending | — | Deployed — l’ouverture directe de l’épisode est configurable sur la rangée All media In Progress et active par défaut; pages en 200 et préflight vert |
